...
output_b <- cbind(bill_cut_a,  weight_category_max)

#write.csv(output_b, file = "output_b.csv")

################################# construction zone ##################################
# just putting the dataset cut here so that we can build the CSV for tomorrow

output_b <- subset(output_b, uplift_service %in% c("Express.VIC"))
...
col_name_max<- as.character(output_b$weight_category_max)
col_index_max <- unlist(lapply(output_b$weight_category_max, function(col_name_max) {
  which(colnames(cz_post_feb_eparcel_express_ex_mel) == col_name_max)
}))

row_name_max<- as.character(output_b$CHARGE.ZONE)
...
output_b_2 <-cbind(output_b, (cbind(row_index_max, col_index_max)))
...
output_b_2$charge_value_max <- mapply(extract_charge_value_max, output_b_2$row_index_max, output_b_2$col_index_max)
...
output_b_2$final_charge_incgrt22 <- mapply(calculate_final_charge, output_b_2$charge_value_max, output_b_2$weight_category_max, output_b_2$max_weight, output_b_2$row_index_max)


#write.csv(output_b_2, file = "output_b_2.csv")

##### multiply by customer uplift   ####
# first step is to find the indices 

# service
col_name_uplift<- as.character(output_b_2$uplift_service)
col_index_uplift <- unlist(lapply(output_b_2$uplift_service, function(col_name_uplift) {
  which(colnames(customer_uplift_march_24) == col_name_uplift)
}))

# customer
row_name_uplift<- as.character(output_b_2$customer_code)
row_index_uplift <- unlist(lapply(output_b_2$customer_code, function(row_name_uplift) {
...
output_b_2 <-cbind(output_b_2, (cbind(row_index_uplift, col_index_uplift)))
...
output_b_2$charge_value_uplift <- mapply(extract_charge_value_uplift, output_b_2$row_index_uplift, output_b_2$col_index_uplift)

#write.csv(output_b_2, file = "output_b_2.csv")

#### multiply base by uplift ####
# Convert charge_value_uplift to numeric, handling NA values
output_b_2$charge_value_uplift_numeric <- ifelse(is.na(output_b_2$charge_value_uplift) | is.na(output_b_2$final_charge_incgrt22),
                                                 NA,
                                                 as.numeric(sub("%", "", output_b_2$charge_value_uplift)))

# Convert final_charge_incgrt22 to numeric, handling NA values
output_b_2$charge_value_max_numeric <- ifelse(is.na(output_b_2$charge_value_uplift) | is.na(output_b_2$final_charge_incgrt22),
                                              NA,
                                              as.numeric(gsub("[^0-9.]", "", output_b_2$final_charge_incgrt22)))

# Calculate the percentage of final_charge_incgrt22, handling NA values
output_b_2$uplift_figure <- ifelse(is.na(output_b_2$charge_value_uplift_numeric) | is.na(output_b_2$charge_value_max_numeric),
                                   NA,
                                   (output_b_2$charge_value_uplift_numeric / 100) * output_b_2$charge_value_max_numeric)

# Filter out NA and non-numeric values before performing addition
output_b_2$charge_to_custo <- ifelse(is.na(output_b_2$charge_value_max_numeric) | is.na(output_b_2$uplift_figure) | !is.numeric(output_b_2$charge_value_max_numeric) | !is.numeric(output_b_2$uplift_figure),
                                     NA,
                                     output_b_2$charge_value_max_numeric + output_b_2$uplift_figure)


# update table name
output_b_3_express_VIC <- output_b_2

##### write to CSV ####

write.csv(output_b_3_express_VIC, file = "output_b_3_express_VIC.csv")